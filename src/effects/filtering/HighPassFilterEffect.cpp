...
HighPassFilterEffect::createParameterLayout() {
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        ...
        200.0f)); // default lower than LP: useful for removing low-end rumble
...
HighPassFilterEffect::HighPassFilterEffect()
    : AudioEffectBase() {
    ...
    DBG("[HighPassFilterEffect] Constructed. cutoff="
...
void HighPassFilterEffect::prepareToProcess(const juce::dsp::ProcessSpec &spec) {
...
void HighPassFilterEffect::resetState() {
    for (auto &f: m_filters)
        f.reset();
}

void HighPassFilterEffect::processBlock(juce::AudioBuffer<float> &buffer) {
    // -------------------------------------------------------------------------
...
juce::ValueTree HighPassFilterEffect::getState() const {
...
void HighPassFilterEffect::setState(const juce::ValueTree &state) {
